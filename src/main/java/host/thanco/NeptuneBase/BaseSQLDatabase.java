...
public class BaseSQLDatabase implements BaseDatabase {
    ...
    public void initDatabase() {
        try {
        connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/Neptune4590", "Neptune4590", "4xjD2oaffanmZXU8IhdJ");
        Statement s = connection.createStatement();
        ResultSet rs = s.executeQuery("SELECT EXISTS (SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA='Neptune4590' AND TABLE_NAME='tableIDs')");
        rs.next();
        if (!rs.getBoolean(1)) {
